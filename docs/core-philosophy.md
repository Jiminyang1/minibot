...
这份文档解释 `refactor/core-loop` 分支对 core runtime 的重构:改了什么、为什么这么改、背后的判断标准是什么。它不是 API 文档,而是一份"如何看架构"的教材——目标是让你下次自己就能闻出同样的问题。
...
这个形状让 streaming 从"要动五层的大工程"变成"一个模块的活":`LLMClient.chat` 增加返回 delta 迭代器的变体,循环边 fold delta 成完整消息、边 emit 增量事件;CLI/SSE 作为订阅者各自决定怎么渲染增量。事件流是现成的管道,`_append` 是现成的终点。这就是"架构对了,功能变便宜"的具体含义。
